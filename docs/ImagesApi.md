...
# **create_images_costs**
> CostEstimation create_images_costs(org_id, request)
...
with aigentchat.ApiClient(configuration) as api_client:
    ...
    api_instance = aigentchat.ImagesApi(api_client)
    org_id = 'org_id_example' # str | organization ID
    request = aigentchat.EstimateImageGenerationCostRequestDto() # EstimateImageGenerationCostRequestDto | Estimate Image Generation Cost Request DTO

    try:
        # Get the total costs of an image generation request.
        api_response = api_instance.create_images_costs(org_id, request)
        print("The response of ImagesApi->create_images_costs:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling ImagesApi->create_images_costs: %s\n" % e)
```
...
# **get_image**
> AIgencyImage get_image(org_id, image_id)
...
with aigentchat.ApiClient(configuration) as api_client:
    ...
    try:
        # Get an image
        api_response = api_instance.get_image(org_id, image_id)
